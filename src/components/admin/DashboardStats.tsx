    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      {statItems.map((item) => (
        <a
          key={item.name}
          href={item.href}
          className={`group relative overflow-hidden rounded-lg border-2 p-4 transition-all duration-200 hover:shadow-lg hover:scale-105 ${getColorClasses(item.color)}`}
        >
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm font-medium opacity-80 group-hover:opacity-100">
                {item.name}
              </p>
              <p className="text-2xl font-bold group-hover:scale-110 transition-transform">
                {item.value.toLocaleString('pt-BR')}
              </p>
              <p className="text-xs opacity-70 group-hover:opacity-100 mt-1">
                {item.description}
              </p>
            </div>
            <div className="ml-4 flex-shrink-0">
              <div className="rounded-full bg-white bg-opacity-20 p-2 group-hover:bg-opacity-30 transition-all">
                {getIcon(item.icon)}
              </div>
            </div>
          </div>
